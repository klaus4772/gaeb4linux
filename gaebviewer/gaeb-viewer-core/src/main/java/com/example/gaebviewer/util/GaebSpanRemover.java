...
public class GaebSpanRemover {
    ...
    private int removeSpanTags(Element element) {
        ...
        String textContent = getTextContent(element);
            ...
            // Count opening <span ...> tags (case-insensitive)
            Pattern openSpan = Pattern.compile("(?i)<span[^>]*>");
            Matcher openMatcher = openSpan.matcher(cleaned);
            int openCount = 0;
            ...
            // Count closing </span> tags (case-insensitive)
            Pattern closeSpan = Pattern.compile("(?i)</span>");
            Matcher closeMatcher = closeSpan.matcher(cleaned);
            int closeCount = 0;
            ...
            spansRemoved = Math.max(openCount, closeCount);
            if (spansRemoved > 0) {

...
@Component
public class Da86JaxbImporter implements GaebImporterFactory.VersionedGaebImporter {

    private static final Logger LOGGER = LoggerFactory.getLogger(Da86JaxbImporter.class);
    private static final XMLInputFactory XML_INPUT_FACTORY = XMLInputFactory.newFactory();

    /** Maps real-world DA86/3.2 files to the 3.3 schema used for JAXB code generation. */
    private static class NamespaceFilter extends StreamReaderDelegate {
        private static final String OLD_NS = "http://www.gaeb.de/GAEB_DA_XML/DA86/3.2";
        private static final String NEW_NS = "http://www.gaeb.de/GAEB_DA_XML/DA86/3.3";
    ...
    private static synchronized JAXBContext getContext() {
        if (CONTEXT == null) {
            try {
                CONTEXT = JAXBContext.newInstance("com.example.gaebviewer.schema.da86");
            } catch (Exception e) {
                throw new RuntimeException("Failed to create JAXBContext for DA86. Ensure gaeb-schema-da86 is built and ObjectFactory is present.", e);
            }
        }
        return CONTEXT;
    }
    ...
    public GaebProject importGaeb(InputStream inputStream) {
        try {
            ...
            System.out.println("DA86 extracted positions: " + positions.size());

            return project;

        } catch (Exception e) {
            throw new RuntimeException("DA86 JAXB import failed", e);
        }
    }

...
import com.example.gaebviewer.schema.da80.TgGAEB;
...
@Component
public class Da80JaxbImporter implements GaebImporterFactory.VersionedGaebImporter {

    private static final Logger LOGGER = LoggerFactory.getLogger(Da80JaxbImporter.class);
    ...
    private static synchronized JAXBContext getContext() {
        if (CONTEXT == null) {
            try {
                CONTEXT = JAXBContext.newInstance("com.example.gaebviewer.schema.da80");
            } catch (Exception e) {
                throw new RuntimeException("Failed to create JAXBContext for DA80. Ensure gaeb-schema-da80 is built and ObjectFactory is present.", e);
            }
        }
        return CONTEXT;
    }
    ...
    public GaebProject importGaeb(InputStream inputStream) {
        try {
            ...
            System.out.println("DA80 extracted positions: " + positions.size());

            return project;

        } catch (Exception e) {
            throw new RuntimeException("DA80 JAXB import failed", e);
        }
    }
    ...
    private void walkPositions(Object cur, List<String> ancestorPath, List<GaebPosition> out, IdentityHashMap<Object, Boolean> visited) {
        ...
        for (java.lang.reflect.Method m : cur.getClass().getMethods()) {
            ...
            try {
                ...
                LOGGER.debug("Failed to invoke {} on {}", m.getName(), cur.getClass().getName(), ex);
            }
        }
    }
        ...
        for (java.lang.reflect.Method m : obj.getClass().getMethods()) {
            if (m.getParameterCount() == 0 && (m.getName().startsWith("get") || m.getName().startsWith("is")) && !m.getName().equals("getClass")) {
                try {
                    ...
                    LOGGER.debug("extractAllText: failed to invoke {} on {}", m.getName(), obj.getClass().getName(), ex);
                }
            }
        }
        return sb.toString().trim();
        ...
        for (java.lang.reflect.Method m : obj.getClass().getMethods()) {
            if (m.getName().startsWith("get") && m.getName().contains("OutlineText") && m.getParameterCount() == 0) {
                try {
                    ...
                    LOGGER.debug("findAnyOutlineText: failed to invoke {} on {}", m.getName(), obj.getClass().getName(), ex);
                }
            }
        }
        return null;
        ...
        for (java.lang.reflect.Method m : obj.getClass().getMethods()) {
            if (m.getName().startsWith("get") && (m.getName().contains("DetailTxt") || m.getName().contains("CompleteText")) && m.getParameterCount() == 0) {
                try {
                    ...
                    LOGGER.debug("findAnyDetailText: failed to invoke {} on {}", m.getName(), obj.getClass().getName(), ex);
                }
            }
        }
        return null;

...
@Component
public class Da84JaxbImporter implements GaebImporterFactory.VersionedGaebImporter {

    private static final Logger LOGGER = LoggerFactory.getLogger(Da84JaxbImporter.class);
    private static final XMLInputFactory XML_INPUT_FACTORY = XMLInputFactory.newFactory();

    /** Maps real-world DA84/3.2 files to the 3.3 schema used for JAXB code generation. */
    private static class NamespaceFilter extends StreamReaderDelegate {
        private static final String OLD_NS = "http://www.gaeb.de/GAEB_DA_XML/DA84/3.2";
        private static final String NEW_NS = "http://www.gaeb.de/GAEB_DA_XML/DA84/3.3";
    ...
    private static synchronized JAXBContext getContext() {
        if (CONTEXT == null) {
            try {
                CONTEXT = JAXBContext.newInstance("com.example.gaebviewer.schema.da84");
            } catch (Exception e) {
                throw new RuntimeException("Failed to create JAXBContext for DA84. Ensure gaeb-schema-da84 is built and ObjectFactory is present.", e);
            }
        }
        return CONTEXT;
    }
    ...
    public GaebProject importGaeb(InputStream inputStream) {
        try {
            ...
            System.out.println("DA84 extracted positions: " + positions.size());

            return project;

        } catch (Exception e) {
            throw new RuntimeException("DA84 JAXB import failed", e);
        }
    }

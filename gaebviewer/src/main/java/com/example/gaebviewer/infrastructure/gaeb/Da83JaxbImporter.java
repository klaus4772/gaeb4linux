...
public class Da83JaxbImporter implements GaebImporterFactory.VersionedGaebImporter {
    ...
    private static synchronized JAXBContext getContext() {
        if (CONTEXT == null) {
            try {
                CONTEXT = JAXBContext.newInstance("com.example.gaebviewer.schema.da83");
            } catch (Exception e) {
                throw new RuntimeException("Failed to create JAXBContext for DA83. Ensure gaeb-schema-da83 is built and ObjectFactory is present.", e);
            }
        }
        return CONTEXT;
    }
    ...
    public GaebProject importGaeb(InputStream inputStream) {
        try {
            ...
            throw new RuntimeException("DA83 JAXB import failed", e);
        }
    }
